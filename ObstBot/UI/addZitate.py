...
class ZitatLehrer(discord.ui.Modal, title='Lehrer Zitat'):
    ...
    async def on_submit(self , interaction: discord.Interaction):
        try:
            await Lehrer.send(str(self.zitat))
...
class ZitatSchuehler(discord.ui.Modal, title='Schüler Zitat'):
    ...
    async def on_submit(self , interaction: discord.Interaction):
        try:
            await Schueler.send(str(self.zitat))
            await interaction.response.send_message('Dein Zitat wurde Erfolgreich hinzugefügt!', ephemeral=True)
...
class ZitatBetrib(discord.ui.Modal, title='Betribs Zitat'):
    ...
    async def on_submit(self , interaction: discord.Interaction):
        try:
            ...
            await Betrib.send(str(self.zitat))
            await interaction.response.send_message('Dein Zitat wurde Erfolgreich hinzugefügt!', ephemeral=True)
...
class ZitatButtons(discord.ui.View):
    try:
        @discord.ui.button(label="Lehrer Zitat", custom_id="button-1", row=0, style=discord.ButtonStyle.primary, emoji="👩‍🏫")  # the button has a custom_id set
        async def button_callback_one(self,  interaction,_ ):
            await interaction.response.send_modal(ZitatLehrer())

        @discord.ui.button(label="Schüler Zitat", custom_id="button-2", row=0,style=discord.ButtonStyle.primary, emoji="🧑‍🎓")  # the button has a custom_id set
        async def button_callback_tow(self, interaction, _):
            await interaction.response.send_modal(ZitatSchuehler())

        @discord.ui.button(label="Betriebs Zitat", custom_id="button-3", row=0,style=discord.ButtonStyle.primary, emoji="🏗️")  # the button has a custom_id set
